...
// RUN: triton-opt %s -gluon-canonicalize | FileCheck %s --check-prefix=GLUON
...
// CHECK-LABEL: @canonicalize_from_ctas
// GLUON-LABEL: @canonicalize_from_ctas
tt.func @canonicalize_from_ctas(%barrier: !ttg.memdesc<8xi64, #barrier, #smem, mutable>, %pred: i1) {
  // CHECK-NEXT: ttng.barrier_expect %arg0, 16, %arg1 :
  // GLUON-NEXT: ttng.barrier_expect %arg0, 16, %arg1 :
  ttng.barrier_expect %barrier, 16 {from_ctas = 7 : i32}, %pred : !ttg.memdesc<8xi64, #barrier, #smem, mutable>
  // CHECK-NEXT: ttng.barrier_expect %arg0, 16 {from_ctas = 5 : i32}, %arg1 :
  // GLUON-NEXT: ttng.barrier_expect %arg0, 16 {from_ctas = 5 : i32}, %arg1 :
  ttng.barrier_expect %barrier, 16 {from_ctas = 5 : i32}, %pred : !ttg.memdesc<8xi64, #barrier, #smem, mutable>
  // CHECK-NEXT: ttng.arrive_barrier %arg0, 1, %arg1 :
  // GLUON-NEXT: ttng.arrive_barrier %arg0, 1, %arg1 :
  ttng.arrive_barrier %barrier, 1, %pred {from_ctas = 7 : i32} : !ttg.memdesc<8xi64, #barrier, #smem, mutable>
  // CHECK-NEXT: ttng.arrive_barrier %arg0, 1, %arg1 {from_ctas = 5 : i32} :
  // GLUON-NEXT: ttng.arrive_barrier %arg0, 1, %arg1 {from_ctas = 5 : i32} :
